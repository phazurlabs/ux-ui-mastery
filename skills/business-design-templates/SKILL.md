...
> - Usability testing (5-7 participants)
> - Deliverable: Figma file, prototype, test report
>
> **Phase 5: Handoff and Support (Weeks 13-14)**
> - Developer handoff documentation
> - Design QA during implementation (2 sprints)
> - Deliverable: Annotated specs, asset exports, QA feedback

### 1.4 Scope and Deliverables Matrix

Be explicit. Ambiguity breeds scope creep.

| Deliverable | Format | Quantity | Included |
|---|---|---|---|
| User interviews | Video + transcript | 8 | Yes |
| Research synthesis | PDF report | 1 | Yes |
| Competitive audit | Spreadsheet + PDF | 1 | Yes |
| User flows | FigJam / Figma | 5 flows | Yes |
| Wireframes | Figma | 20 screens | Yes |
| Visual directions | Figma | 3 options | Yes |
| High-fidelity screens | Figma | 40 screens | Yes |
| Interactive prototype | Figma prototype | 2 key flows | Yes |
| Usability test report | PDF | 1 | Yes |
| Design system (foundation) | Figma + tokens | 1 | Yes |
| Motion/animation specs | Figma + video | Key interactions | Yes |
| Developer handoff | Figma annotations | All screens | Yes |
| Design QA | Slack/Loom | 2 sprint cycles | Yes |
| Illustration/custom icons | — | — | Add-on |
| Copywriting/UX writing | — | — | Add-on |
| Front-end development | — | — | Out of scope |

### 1.5 Timeline (Gantt/Milestone Format)

**Milestone format (for proposals):**

| Milestone | Target Date | Payment Trigger |
|---|---|---|
| Project kickoff | Week 1 | 30% deposit |
| Research complete | Week 3 | — |
| IA and wireframes approved | Week 5 | 30% milestone |
| Visual direction approved | Week 7 | — |
| High-fidelity designs complete | Week 11 | 30% milestone |
| Handoff and QA complete | Week 14 | 10% final |
...
|---|---|---|---|
| Research | Competitive audit only | Audit + 5 interviews | Audit + 10 interviews + diary study |
| Design | 20 screens, 1 direction | 35 screens, 2 directions | 50+ screens, 3 directions |
| Prototype | Click-through | Interactive (2 flows) | Interactive (all flows) + motion |
| Testing | Heuristic review | 5-participant usability test | Test + iterate + retest |
| Support | Handoff docs | Handoff + 1 sprint QA | Handoff + 3 sprint QA |
| **Investment** | **$XX,000** | **$XX,000** | **$XX,000** |
...
1. **Client Context** — Who they are, their market, their scale. 2-3 sentences.
2. **The Challenge** — The problem in human terms. What was broken, what was the cost. Include a direct quote from the client if possible.
3. **Process and Key Decisions** — Your methodology. Show the messy middle: research that changed your assumptions, design explorations that failed, the pivotal decision that shaped the solution.
4. **The Solution** — Final designs with context. Annotate key decisions. Show before/after if possible.
5. **Measurable Results** — Numbers. Always numbers. Conversion rates, task completion times, NPS scores, revenue impact, support ticket reduction.
6. **Client Testimonial** — A direct quote. Get this before the project ends while satisfaction is high.
...
### Principles
1. [Principle]: [Explanation and example of application]
2. [Principle]: [Explanation and example of application]
3. [Principle]: [Explanation and example of application]
...
| Token | Value | Usage |
|---|---|---|
| --color-primary | oklch(55% 0.22 260) | Primary actions, links |
| --color-primary-hover | oklch(48% 0.22 260) | Hover state for primary |
| --color-surface | oklch(99% 0.005 260) | Page and card backgrounds |
| --color-text-primary | oklch(20% 0.02 260) | Body text, headings |
| --color-border | oklch(88% 0.01 260) | Dividers, input borders |
| --color-error | oklch(55% 0.22 25) | Error states, destructive |
| --color-success | oklch(55% 0.15 155) | Success states, positive |
...
### Components
[For each component: name, description, variants, props, states, usage guidelines,
do/don't examples, accessibility requirements, code snippet]

### Patterns
[Compositions of components: form layouts, card grids, navigation patterns,
page templates. Include when-to-use guidance.]

### Usage Guidelines
- When to use [Component A] vs. [Component B]
- Composition rules (spacing between sections, max content width)
...
## 5. Client Handoff Checklist (Comprehensive)

### Screens and States

- [ ] All unique screens designed (list with count)
- [ ] Loading/skeleton states for every screen with async data
- [ ] Empty states (first-time user, no data, no results)
- [ ] Error states (network error, validation error, server error, 404)
- [ ] Success/confirmation states (form submitted, action completed)
- [ ] Partial/degraded states (offline mode, slow connection)
- [ ] Permission states (logged out, insufficient permissions, paywall)
...
- [ ] Tokens exported as JSON/CSS custom properties
- [ ] Color palette with semantic names (not "blue-500" but "color-primary")
- [ ] Typography scale with responsive values
- [ ] Spacing scale documented
- [ ] Border radius, shadow, and elevation scales
- [ ] Motion/duration tokens (timing functions, durations)
- [ ] Breakpoint tokens

### Assets

- [ ] Icons exported as SVG (optimized, consistent viewBox)
- [ ] Illustrations exported at 1x, 2x, 3x (or SVG)
- [ ] Photographs optimized (WebP with JPEG fallback, srcset sizes documented)
- [ ] Favicons: 16x16, 32x32, 180x180 (Apple touch), 192x192 (Android), 512x512 (PWA)
- [ ] OG/social images (1200x630)
- [ ] App icons (if applicable): full set for target platforms
- [ ] Lottie/JSON animation files (if applicable)
- [ ] Font files or CDN links with license confirmation

### Responsive Specifications

- [ ] Layout grid defined per breakpoint (columns, gutters, margins)
- [ ] Stacking/reflow behavior documented for each major section
- [ ] Touch target sizes verified (minimum 44x44px on mobile)
- [ ] Font size scaling documented (fluid or breakpoint-based)
- [ ] Navigation pattern changes documented (e.g., top bar to bottom tabs)
- [ ] Image behavior documented (crop, scale, art direction changes)

### Animation and Motion

- [ ] Transition specs: property, duration, easing function
- [ ] Page transition pattern (shared element, fade, slide)
- [ ] Micro-interaction specs (button press, toggle, checkbox)
- [ ] Loading animation specs (skeleton shimmer direction, speed)
- [ ] Scroll-triggered animation specs (threshold, direction)
- [ ] Reduced-motion alternatives for all animations

### Accessibility

- [ ] Color contrast ratios verified (APCA or WCAG 2.2 AA minimum)
- [ ] Focus order documented for complex layouts
- [ ] ARIA labels specified for icons, images, and custom controls
- [ ] Heading hierarchy documented (h1-h6 per page)
- [ ] Landmark regions identified (header, main, nav, footer)
...
- [ ] Pages organized: Cover, Flows, Components, Screens, Handoff, Archive
...
**Rate calculation:** (Annual salary target / 48 weeks / 25 billable hours) x 1.3 (overhead)
...
|---|---|---|---|
| Junior (0-2 yrs) | $50-80 | $400-640 | $4,000-8,000 |
| Mid (3-5 yrs) | $80-150 | $640-1,200 | $8,000-16,000 |
| Senior (6-10 yrs) | $150-250 | $1,200-2,000 | $16,000-30,000 |
| Lead/Principal (10+ yrs) | $250-400 | $2,000-3,200 | $30,000-50,000 |
| Agency/Studio | $150-350 | $1,200-2,800 | $15,000-50,000 |
...
|---|---|
| "I don't like the color" | "What feeling should it evoke? Let's align on intent." |
| "My spouse thinks..." | "I appreciate that perspective. Let's check it against our user research." |
| "Can we make it pop?" | "Can you show me an example of what 'pop' means to you?" |
| "The competitor does X" | "Yes — let's look at why. Our research suggests our users need Y." |
| "Just make it like Apple" | "Which aspect? Simplicity, animation, information density? Let's be specific." |
| "We need to add [feature]" | "Great idea. Let's capture it. Where does it rank against our current priorities?" |
...
1. **Definition of Confidential Information** — Include: designs, research data, business strategy, user data, financial terms. Exclude: publicly available information, information known prior to engagement.
2. **Duration** — Typically 2-3 years for design work. Perpetual NDAs are a red flag.
3. **Permitted Disclosure** — Subcontractors (with their own NDA), legal requirements, employees who need to know.
4. **Return/Destruction** — Upon termination, return or destroy confidential materials within 30 days.
5. **Portfolio Exception** — Negotiate this upfront: "Designer may display final deliverables in portfolio after public launch, subject to Client approval."

### Design Contract Essentials (Must-Have Clauses)

1. **Scope of Work** — Reference the SOW document. Include change order process.
2. **Payment Terms** — Amounts, schedule, method, late fees. Specify that work pauses if payment is 15+ days overdue.
3. **Intellectual Property Transfer** — IP transfers only upon full payment. Before that, designer retains all rights.
4. **Revisions and Approvals** — Number of rounds, definition of a "round," what happens after the limit.
5. **Timeline and Delays** — Client-caused delays extend timeline 1:1. Define "business days" for feedback windows.
6. **Termination** — Kill fee (typically 25-50% of remaining project value). Payment for all completed work.
7. **Liability Cap** — Limit liability to the total project fee. Never accept unlimited liability.
8. **Indemnification** — Client provides content/assets legally. You are not liable for client-supplied IP infringement.
9. **Portfolio Rights** — Right to display work in portfolio, on social media, and in case studies after launch.
10. **Independent Contractor Status** — You are not an employee. Specify tax responsibility.
...
**The Solution (Visual-Heavy):**
- 3-5 annotated screens showing key flows
- Before/after comparison if applicable
- Detail shots showing craft (spacing, states, micro-interactions)
- Show responsive behavior

**The Results (Quantified):**
- 2-3 metrics with before/after numbers
- Client testimonial quote
- Lessons learned (shows self-awareness)

### Portfolio Tips for Hiring Managers

- Put your strongest case study first and your second strongest last (primacy and recency effect)
- Lead every case study with the result, not the process
- Show 3-5 projects maximum (quality over quantity)
- Include at least one shipped project with real data
- Show range: one consumer, one enterprise, one conceptual
- Label your exact contribution on team projects
- Make the portfolio itself a demonstration of your design skill
- Mobile-optimize your portfolio (recruiters browse on phones)
- Load time under 3 seconds (performance is a design decision)
- Include a clear, prominent "contact me" CTA
